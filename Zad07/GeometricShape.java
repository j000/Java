...
class GeometricShape implements GeometricShapeInterface {
	...
	/**
	 * Metoda usuwa punkt, o ile taki istnieje. Jeśli w kolekcji punktów jest
	 *więcej takich samych jak point, usuwany jest tylko pierwszy z nich.
	 *
	 * @param point punkt do usunięcia
	 * @return true - punkt istniał w kolekcji i został usunięty, false -
	 *takiego punktu nie było i w związku z tym nie został usunięty.
	 */
	 ...
	 *		 false - wskazanego punktu odniesienia nie było, dodanie nowego
	 *punktu nie było możliwe.
	 */
	 ...
	 *		 zwracana jest referencja do usuniętego punktu. Gdy punkt
	 *odniesienia lub punkt przed nim nie istniał zwracany jest NULL.
	 */
	 ...
	 *
	 * @param afterPoint punkt istniejący bezpośrednio po afterPoint należy
	 *usunąć.
	 * @return Gdy punkt odniesienia istniał oraz istniał punkt do usunięcia,
	 *		 zwracana jest referencja do usuniętego punktu. Gdy punkt
	 *odniesienia lub punkt za nim nie istniał zwracany jest NULL.
	 */
	...
	/**
	 * Metoda usuwa skutek ostatniego polecenia add, remove, addAfter,
	 *addBefore, removeAfter lub removeBefore. Poprzez wielokrotne wykonywanie
	 *metody undo() możliwe jest usunięcie wcześniejszych poleceń. Usunięciu
	 *podlegają wyłącznie polecenia poprawnie zrealizowane, tych, które nie
	 *zmieniły stanu listy punktów nie należy zapamietywać i uwzględniać w
	 *działaniu undo.
	 *
	 ...
	 * bezpośrednio poprzedza wykonanie redo (uwaga: operacje odczytu stanu
	 *obiektu mogą pojawić się pomiędzy undo a redo, podobnie jak inne operacje,
	 *które nie zmienią stanu obiektu czyli np. błęde zlecnie addBefore)
	 *
	...
	/**
	 * Metoda zwraca listę punktów bez kolejnych powtórzeń. Kolejność punktów w
	 * tej kolekcji ma być taka sama, jak w przypadku list zwracanej za pomocą
	 * metody get jednakże bez powtórzeń. Jeśli kolejność punktów zwracana za
	 * pomocą get to np. P1, P2, P2, P3, P2, P1, P1, P1, P4 to metoda getUniq ma
	 * zwrócić: P1, P2, P3, P2, P1 i P4, czyli usuwane są wszystkie kolejne
	 * wystąpienia danego punktu. Powtórzenie punktu ma miejsce wtedy, gdy
	 * P1.equals(P2)=true. Uwaga: metoda zwraca listę (nie zbiór) punktów, wciąż
	 * jeden punkt może wystąpić wielokrotnie w wyniku.
	 *
